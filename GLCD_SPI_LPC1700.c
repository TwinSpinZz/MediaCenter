...
// Add this to your GLCD.c file
void GLCD_DrawRect(unsigned int x, unsigned int y, unsigned int width, unsigned int height) {
    int i, j;
		for (i = 0; i <= width; i++) {
        GLCD_PutPixel(x + i, y);          // Top line
        GLCD_PutPixel(x + i, y + height); // Bottom line
    }
    for (j = 0; j <= height; j++) {
        GLCD_PutPixel(x, y + j);          // Left line
        GLCD_PutPixel(x + width, y + j);  // Right line
    }
}
